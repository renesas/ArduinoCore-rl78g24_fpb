...
#define TRIM_DURATION       2                               // compensation ticks to trim adjust for digitalWrite
#elif defined(G24_FPB)
#define usToTicks( _us )    ( (uint16_t)( _us * 1.5 )  )      // converts microseconds to tick (assumes prescale of 8)   12 Aug 2009
#define ticksToUs( _ticks ) ( (uint16_t)( (_ticks * 2) / 3 ) ) // converts from ticks back to microseconds
#define TRIM_DURATION       2                               // compensation ticks to trim adjust for digitalWrite delays
/*
uint16_t ticksToUs(uint16_t _ticks){
	return (uint16_t)(_ticks / 1.5);
}
*/
#endif  // G22/G23/G24
...
#elif defined(G24_FPB)
/* Declaration by cores/rl78g23/smc_gen/Config_TAU0_x_Servo/Config_TAU0_x_Servo_user.c */
extern void (*g_fServoInterruptFunc0)(void);
extern void (*g_fServoInterruptFunc2)(void);
extern void (*g_fServoInterruptFunc3)(void);
#endif  // G22/G23/G24
...
//        temp_TDR = (uint16_t)(temp_TDR * 1.5);
        Reset_TDR(temp_TDR, timer);
...
#endif
#if defined(G24_FPB)
static void g_fServoInterruptFunc0_sub(void)
{
    servo_handle_interrupts(_timerTAU00);
}
...
static void g_fServoInterruptFunc3_sub(void)
{
    servo_handle_interrupts(_timerTAU03);
}
#endif
...
uint8_t Servo::attach(int pin, int min_a, int max_a)
{
    if(this->servoIndex < MAX_SERVOS )
    {
        ...
        if(timer == 0)
        {
            if(isTimerActive(timer) == false)
            {
                g_fServoInterruptFunc0 = g_fServoInterruptFunc0_sub;
                Channel[timer] = -1;
        ...
        else
        {
            if(isTimerActive(timer) == false)
            {
                g_fServoInterruptFunc3 = g_fServoInterruptFunc3_sub;
                Channel[timer] = -1;
