...
***********************************************************************************************************************/
volatile uint8_t   g_iic21_master_status_flag;    /* iic21 start flag for send address check */
volatile uint8_t   g_iica21_master_status_flag;   /* iic21 master flag */
volatile uint8_t * gp_iic21_tx_address;           /* iic21 send data pointer by master mode */
volatile uint16_t  g_iic21_tx_cnt;              /* iic21 send data size by master mode */
volatile uint8_t * gp_iic21_rx_address;           /* iic21 receive data pointer by master mode */
volatile uint16_t  g_iic21_rx_cnt;              /* iic21 receive data size by master mode */
uint16_t  g_iic21_rx_len;                      /* iic21 receive data length by master mode */
/* Start user code for global. Do not edit comment generated here */
/* End user code. Do not edit comment generated here */

/***********************************************************************************************************************
* Function Name: R_Config_IIC21_Create
* Description  : This function initializes the IIC21 master module.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
void R_Config_IIC21_Create(void)
{
    ...
    SCR11 = _0004_SAU_SCRMN_INITIALVALUE | _0000_SAU_TIMING_1 | _0000_SAU_INTSRE_MASK | _0000_SAU_PARITY_NONE | 
            _0000_SAU_MSB | _0010_SAU_STOP_1 | _0003_SAU_LENGTH_8;
    SDR11 = _EE00_IIC21_DIVISOR;
    ...
    R_Config_IIC21_Create_UserInit();
}

/***********************************************************************************************************************
* Function Name: R_Config_IIC21_Stop
* Description  : This function stops the IIC21 module.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
void R_Config_IIC21_Stop(void)
{
    IICMK21 = 1U;    /* disable INTIIC21 interrupt */
    ST1 |= _0002_SAU_CH1_STOP_TRG_ON;
    IICIF21 = 0U;    /* clear INTIIC21 interrupt flag */
...
/***********************************************************************************************************************
* Function Name: R_Config_IIC21_StartCondition
* Description  : This function starts the IIC21 condition.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
void R_Config_IIC21_StartCondition(void)
{
    volatile uint8_t w_count;

    SO1 &= (uint16_t)~_0002_SAU_CH1_DATA_OUTPUT_1;    /* clear IIC21 SDA */
    ...
    SO1 &= (uint16_t)~_0200_SAU_CH1_CLOCK_OUTPUT_1;    /* clear IIC21 SCL */
...
/***********************************************************************************************************************
* Function Name: R_Config_IIC21_StopCondition
* Description  : This function stops the IIC21 condition.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
void R_Config_IIC21_StopCondition(void)
{
    volatile uint8_t w_count;

    ST1 |= _0002_SAU_CH1_STOP_TRG_ON;    /* disable IIC21 */
    SOE1 &= (uint16_t)~_0002_SAU_CH1_OUTPUT_ENABLE;    /* disable IIC21 output */
    SO1 &= (uint16_t)~_0002_SAU_CH1_DATA_OUTPUT_1;    /* clear IIC21 SDA */
...
/***********************************************************************************************************************
* Function Name: R_Config_IIC21_Master_Send
...
* Return Value : None
***********************************************************************************************************************/
MD_STATUS R_Config_IIC21_Master_Send(uint8_t adr, uint8_t * const tx_buf, uint16_t tx_num, uint8_t wait)
{
    ...
    /* Set paramater */
    g_iic21_tx_cnt = tx_num;
    gp_iic21_tx_address = tx_buf;
    /* Start condition */
    R_Config_IIC21_StartCondition();
    IICIF21 = 0U;    /* clear INTIIC21 interrupt flag */
    IICMK21 = 0U;    /* enable INTIIC21 */
    SIO21 = adr;
    g_iica21_master_status_flag = _00_IICA_MASTER_FLAG_CLEAR;
    return (status);
}

/***********************************************************************************************************************
* Function Name: R_Config_IIC21_Master_Receive
...
* Return Value : None
***********************************************************************************************************************/
MD_STATUS R_Config_IIC21_Master_Receive(uint8_t adr, uint8_t * const rx_buf, uint16_t rx_num, uint8_t wait)
{
    MD_STATUS status = MD_OK;
 ...
 * Return Value : None
 *********************************************************************************************************************/
void R_Config_IIC21_Master_SetClock(uint32_t clock) {
